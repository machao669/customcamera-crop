...
package com.machao.camera.camerahelp;
...
public final class CameraManager {
    ...
    public PlanarYUVLuminanceSource buildLuminanceSource(byte[] data, int width, int height, Rect screenEffecRect) {
        if (screenEffecRect == null) {
            return buildLuminanceSource(data, width, height);
        }

        //相机的rect是90度旋转的,所以用x/y求得比例
        float t = (float) configManager.getCameraResolution().x / configManager.getScreenResolution().y;
        return new PlanarYUVLuminanceSource(data, width, height, (int) (screenEffecRect.top * t), (int) (screenEffecRect.left * t), (int) (screenEffecRect.height() * (t + 0.2)), (int) (screenEffecRect.width() * t), false); // Search QR in all image along, not only in Framing Rect as original code done
